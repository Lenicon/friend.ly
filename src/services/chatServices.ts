...
export const createConversationAsync = async (userId, friendId) => {
    try {
        ...
        const conv = {
            ...
            last: { message: "XXCONVO_FIRSTMESSAGEXX", createdAt: serverTimestamp() },
    ...
    try {
        ...
        if (messageId) {
            ...
            await updateDoc(convDoc, {
                last: {
                    message: (msg.message == "" && msg.images.length > 0) ? `XXMSGPHOTOPROCESSSENDXX__${msg.images.length}__${message?.sender}`:msg.message,
...
export const getMsgQueryByConversationId = (convId, limitSize = 20) => {
    return query(
...
export const countMsgs = async(convId) => {
    const coll = collection(db, "conversations", convId, "messages");
